...
print "Reading CogRes Study"
study = Study('/share/studies/CogRes', 'CogRes')
os.chdir(study.path)
studydata = databin()
studydata.maintainer = 'Nikhil Chandra'
studydata.maintainer_email = 'nsc2124@columbia.edu'
studydata.dcmqr_object = dcmqr_wrapper(dcmqr_path='dcmqr', dcm4chee_inst='TAUB_CHEE@156.145.15.135:11112', dcmqr_inst='CogRes_QR@10.115.15.198:11139', dcmrcv_inst='CogRes')
#studydata.dcmqr_args_list = [studydata.dcmqr_object.get_dcmqr_args(PatientName='*COGRES*'),
#                             studydata.dcmqr_object.get_dcmqr_args(PatientName='*BAY0596*')]
studydata.dcmqr_args_list = [studydata.dcmqr_object.get_dcmqr_args(PatientName='*COGRES*')]
...
format_CSVs(['CSV/Subject_List/CogRes_Subjects.csv'], 'CSV/Subject_List/ref_list.csv', accepted_headings=['MRI1', 'MRI2', 'MRI3', 'PET'])
...
def study_generate_merged_sheet(To=[]):
    if type(To) == str:
        To = [To]
    check_make_vars()
...
def study_email_merged_sheet(To=[]):
    ...
    check_sheet_path = os.path.join(email_csv_dir, '{}_check_series.xls'.format(study.name))
    print "creating merged series spreadsheet"
    if os.path.exists(merged_sheet_path):
        os.unlink(merged_sheet_path)
    if os.path.exists(check_sheet_path):
