...
class DICOMseries_read(object):
    ...
    def __init__(self, study, DICOMseries2study_subid, DICOMseries2study_visid, DICOMseries2study_series_types, assign_series, dcmqr_args_list):
        ...
        print "\tLoading Chee Series"      # THIS LOOKS UP THE DICOM SERVER FOR ALL COGRES SCANS
        ...
        print "\tLoading Sorted Series"    # THIS LOOKS UP THE CLUSTER FOR ALL COGRES SCANS
        sorted_series = self.load_sorted_series()
        series_list = [unsorted_series, sorted_series] + chee_series_list   # LIST OF DICTIONARIES. FIRST == UNSORTED. SECOND == SORTED. THIRD == DICOM SERVER
        print "\tMerging Dictionaries"     # THIS COMPARES THE CONTENTS OF THE CLUSTER WITH THE CONTENTS OF THE DICOM SERVER
    ...
    def merge_dictionaries(self, dict_list, prune_paths=False):
        ...
        for DICOMseries_dict in dict_list:          # dict_list contains 3 dictionaries - FIRST == UNSORTED.. SECOND == SORTED.  THIRD == DICOM SERVER.
